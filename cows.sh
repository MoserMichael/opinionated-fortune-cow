...
function run_cow {  
    ...
    # 5. call nltk to do the sentiment analysis
    # 6. leave out the number after compound: 
    # 7. normalize to range of -100..100

    score=$(cat /tmp/cow | tr '\n' ' ' | tr -dc '[:print:]' | sed -e 's/\(.*\)--\(.*\)$/\1/g' | $ENV_DIR/n.py  2>/dev/null |  sed -e 's/compound: \([0-9\.\-]*\).*/\1/' |  awk '{printf "%.0f\n", $1 * 100 }')
